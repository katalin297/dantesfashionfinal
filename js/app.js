...
function showTotals() {
    ...
    const totalMoney = total.reduce((acc, curr) => {
        return (acc += curr);
    }, 0);
...
function eventListeners() {
    ...
    cartInfo.addEventListener('click', event => {
        if (event.target.classList.contains('cart-info') || event.target.parentElement.classList.contains('cart-info') || event.target.parentElement.parentElement.classList.contains('cart-info')) {
            ui.showCart();
        }
    });
    ...
    cartBtn.forEach(btn => {
        btn.addEventListener('click', event => {
            if (event.target.parentElement.classList.contains('store-item-icon') || event.target.classList.contains('store-item-icon')) {
